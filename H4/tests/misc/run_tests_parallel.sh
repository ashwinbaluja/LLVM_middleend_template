...
for i in `ls | grep test` ; do
  ...
  if test "$testSucceeded" != ""; then
    ...
    if ! test -f output/oracle.txt ; then
      echo "ERROR: oracle output \"`pwd`/output/oracle.txt\" is missing" ;
      exit 1;
    fi
    oracleCost=`tail -n 1 output/*.txt | awk '{print $4}'` ;
    if [[ $oracleCost =~ ^[0-9]+$ ]]; then
    ...
    incorrectSolution=`grep "isn't correct" tmpOut` ;
    suboptimalSolution=`grep "there are too many CAT invocations left" tmpOut` ;
    timeoutSolution=`grep "Terminated" tmpOut` ;
    abortedSolution=`grep "Aborted" tmpOut` ;
    segSolution=`grep "Segmentation fault" tmpOut` ;
    if test "$incorrectSolution" != ""; then
      testFailed="  $i [incorrect]\n$testFailed" ;
    elif test "$suboptimalSolution" != ""; then
      testFailed="  $i [suboptimal]\n$testFailed";
    elif test "$timeoutSolution" != "" ; then
      testFailed="  $i [timeout]\n$testFailed";
    elif test "$abortedSolution" != "" ; then
      testFailed="  $i [aborted]\n$testFailed";
    elif test "$segSolution" != "" ; then
      testFailed="  $i [segmentation fault]\n$testFailed";
    else
      testFailed="  $i\n$testFailed";
    fi
  fi
